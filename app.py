# Version 0.181.06-release
import os
import gradio as gr
from TTS.api import TTS
import torch
import time
from pydub import AudioSegment, effects

# नियमों को स्वीकार करना
os.environ["COQUI_TOS_AGREED"] = "1"
device = "cuda" if torch.cuda.is_available() else "cpu"

# AI मॉडल लोड करना
print(f"🚀 सिस्टम {device} पर चालू है...")
try:
    tts = TTS("tts_models/multilingual/multi-dataset/xtts_v2").to(device)
except Exception as e:
    print(f"मॉडल लोड एरर: {e}")

def process_audio(file_path, remove_silence):
    if not remove_silence:
        return file_path
    
    # सन्नाटा हटाने की प्रक्रिया
    audio = AudioSegment.from_wav(file_path)
    # यह सन्नाटे को पहचान कर उसे काट देता है
    audio = effects.strip_silence(audio, silence_thresh=-40, padding=100)
    audio.export(file_path, format="wav")
    return file_path

def generate_voice(voice_sample, script, emotion, speed, language, remove_silence):
    if not voice_sample or not script:
        return None, "❌ गलती: सैंपल और स्क्रिप्ट डालें!"
    
    # शब्दों को अटकने से रोकने के लिए टेक्स्ट की सफ़ाई
    clean_text = " ".join(script.split())
    output_path = f"vbs_output_{int(time.time())}.wav"
    
    try:
        # AI आवाज़ जनरेशन (बिना हकलाए)
        tts.tts_to_file(
            text=clean_text,
            speaker_wav=voice_sample,
            language=language,
            file_path=output_path,
            emotion=emotion,
            speed=speed,
            enable_text_splitting=True
        )
        
        # सन्नाटा हटाना
        final_file = process_audio(output_path, remove_silence)
        
        return final_file, f"✅ सफलता! शब्द: {len(script.split())}"
    except Exception as e:
        return None, f"❌ AI एरर: {str(e)}"

def count_words(text):
    words = len(text.split())
    return f"शब्दों की संख्या: {words} / 10,000"

# डार्क और लाइट मोड के साथ मॉडर्न इंटरफ़ेस
with gr.Blocks(theme=gr.themes.Soft(primary_hue="cyan", neutral_hue="slate")) as demo:
    gr.Markdown("# 🎙️ **वॉइस-बैच स्टूडियो प्रो v0.181.06**")
    
    with gr.Row():
        with gr.Column(scale=1):
            gr.Markdown("### ⚙️ **कंट्रोल सेटिंग्स**")
            voice_in = gr.Audio(label="वॉयस सैंपल अपलोड करें", type="filepath")
            
            with gr.Row():
                lang_opt = gr.Dropdown(choices=["hi", "en", "es", "fr"], value="hi", label="🌍 भाषा")
                emotion_opt = gr.Dropdown(choices=["Neutral", "Sad", "Happy", "Angry", "Surprise"], value="Neutral", label="🎭 भावना (Emotion)")
            
            speed_sl = gr.Slider(0.5, 2.0, 1.0, step=0.1, label="⏩ आवाज़ की गति (Speed)")
            silence_btn = gr.Checkbox(label="🤫 सन्नाटा हटाएं (Silence Remover)", value=True)
            
            gen_btn = gr.Button("🚀 आवाज़ जेनरेट करें", variant="primary")
            status = gr.Textbox(label="सिस्टम स्टेटस", interactive=False)

        with gr.Column(scale=2):
            word_count_display = gr.Markdown("शब्दों की संख्या: 0 / 10,000")
            script_in = gr.Textbox(label="अपनी कहानी यहाँ लिखें", lines=15, placeholder="स्क्रिप्ट पेस्ट करें...")
            
            script_in.change(count_words, inputs=[script_in], outputs=[word_count_display])
            
            gr.Markdown("### 🎧 **आउटपुट (सुनें और डाउनलोड करें)**")
            audio_out = gr.Audio(label="तैयार आवाज़", type="filepath")

    gen_btn.click(
        generate_voice, 
        [voice_in, script_in, emotion_opt, speed_sl, lang_opt, silence_btn], 
        [audio_out, status]
    )

demo.launch(share=True)
