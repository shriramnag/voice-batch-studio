# VoiceBatch Studio Pro - Voice Encoder (Voice Identity Module)
...
# आपके फोल्डर स्ट्रक्चर के हिसाब से इम्पोर्ट्स
...
class VoiceEncoder(nn.Module):
    def __init__(self, hp=VoiceEncConfig()):
        super().__init__()
        self.hp = hp
        # LSTM लेयर्स जो आवाज़ के पैटर्न को समझती हैं
    ...
    def forward(self, mels: torch.FloatTensor):
        if self.hp.normalized_mels and (mels.min() < 0 or mels.max() > 1):
            raise Exception("Mels outside [0, 1]")
    ...
    def embeds_from_wavs(self, wavs: List[np.ndarray], sample_rate, as_spk=False, batch_size=32, trim_top_db: Optional[float]=20, **kwargs):
        if sample_rate != self.hp.sample_rate:
            wavs = [librosa.resample(wav, orig_sr=sample_rate, target_sr=self.hp.sample_rate, res_type="kaiser_fast") for wav in wavs]
        if trim_top_db:
